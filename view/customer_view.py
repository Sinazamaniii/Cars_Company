...
class CustomerView:
    def __init__(self):
        ...
        if not Session.employee:
            self.address.label.place_forget()
            self.address.entry.place_forget()
    ...
    def save_click(self):
        address_value = self.address.get() if Session.employee else ""
    ...
    def edit_click(self):
        address_value = self.address.get() if Session.employee else ""
        status, message = CustomerController.update(
            self.id.get(),
            self.customer_id.get(),
            self.first_name.get(),
            self.last_name.get(),
